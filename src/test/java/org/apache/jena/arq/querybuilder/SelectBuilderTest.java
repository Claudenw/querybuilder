...
public class SelectBuilderTest extends AbstractRegexpBasedTest{
	...
	public void testSelectAsterisk()
	{
		...
		assertContainsRegex( SELECT+"\\*"+SPACE+WHERE+OPEN_CURLY+var("s")+SPACE+var("p")+SPACE+var("o")+OPT_SPACE+DOT+CLOSE_CURLY,
				builder.buildString());
		
		builder.setVar( Var.alloc("p"), RDF.type);
		
		assertContainsRegex( SELECT+"\\*"+SPACE+WHERE+OPEN_CURLY+var("s")+SPACE+node("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")+
...
ORDER BY ?s
		 ...
		 */
		assertContainsRegex( PREFIX+"foaf:"+SPACE+node("http://xmlns.com/foaf/0.1/"), query);
		assertContainsRegex( SELECT+var("s"), query);
		assertContainsRegex( WHERE+OPEN_CURLY+var("s")+SPACE+
				node("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")+SPACE+"foaf:Person"+OPT_SPACE+DOT+SPACE
				+OPTIONAL+OPEN_CURLY+var("s")+SPACE+"foaf:name"+SPACE+var("name")+SPACE+DOT+OPT_SPACE+CLOSE_CURLY
				+CLOSE_CURLY, query );
		...
		assertContainsRegex( WHERE+OPEN_CURLY+var("s")+SPACE+
				node("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")+SPACE+"foaf:Person"+OPT_SPACE+DOT+SPACE
				+OPTIONAL+OPEN_CURLY+var("s")+SPACE+"foaf:name"+SPACE+quote("Smith")+"\\^\\^"+node("http://www.w3.org/2001/XMLSchema#string")+SPACE+DOT+OPT_SPACE+CLOSE_CURLY
				+CLOSE_CURLY, query );
		assertContainsRegex( ORDER_BY+var("s"), query);
	}
	...
	public void testPredicateVar()
	{
        ...
        assertContainsRegex( WHERE+OPEN_CURLY+":S"+SPACE+
				var("p")+SPACE+":O"+OPT_SPACE+DOT+SPACE
				+CLOSE_CURLY, query );
	}
	...
	public void testObjectVar()
	{
        ...
        assertContainsRegex( WHERE+OPEN_CURLY+":S"+SPACE+
				":P"+SPACE+var("o")+OPT_SPACE+DOT+SPACE
				+CLOSE_CURLY, query );
	}
